...
export class SelectedFilterStateService {
  ...
  addFilter(filter: Filter | Filter[]): void {
    ...
    addedFilter = addedFilter.filter(
      (added) => !columnFilter.some((existing) => existing.id === added.id)
    );
  ...
  removeFilter(filter: Filter | Filter[]): void {
    ...
    removedFilter.forEach((removed) => {
      ...
      const columnFilter = this.filterList.get(column) ?? [];
      const updatedColumnFilter = columnFilter.filter(
        (existingFilter) => existingFilter.id !== removed.id
      );
  ...
  removeAllFilter(): void {
    const removedFilter = Array.from(this.filterList.values()).reduce(
      (acc, curr) => {
        acc.push(...curr);
        return acc;
      },
