...
export class DataSourceService<T extends Datasource<T>> {
  ...
  readonly dataSourceChanged$ = this.dataSourceChanged
    .asObservable()
    .pipe(shareReplay());
    ...
    groupedFilter.forEach((columnFilter, column) => {
      ...
      switch (filterType) {
        ...
        case FilterType.DATE_FILTER:
          filterDate = columnFilter
            .map((filter) => filter.value as FilterDate)
