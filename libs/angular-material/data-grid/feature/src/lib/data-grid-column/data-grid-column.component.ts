...
export class DataGridColumnComponent<T extends Datasource<T>>
  ...
  constructor(
    ...
    this.selectedFilterService.removedFilter$
      .pipe(
        ...
        map((removedFilter) =>
          removedFilter.filter((filter) => filter.column === this.column)
        )
      )
      .subscribe((filter) => {
      ...
      .subscribe(() => {
        switch (this.type) {
          case FilterType.CHECK_FILTER:
            this.checkFilter = this.dataSourceService.getCheckFilter(
              this.column,
              this.label
            );
            break;
